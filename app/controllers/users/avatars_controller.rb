  ...
  def destroy
    ...
    respond_to do |format|
      format.turbo_stream { render "avatar/remove_image",
                                   locals: {avatar: "avatar_#{@user.id}", instance_avatar: user_avatar,
                                            instance: @user, partial: "users/user",  chat_header:, avatar_big:,
                                            user_avatar:,
                                            flash_notice: "Cover image was successfully removed from your data!"}
      }
    end
  end
